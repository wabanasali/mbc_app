<?php 
	...
	function cash_in_tithes($christian_id, $amount){
		...
		if ($rows > 0) {
			...
			$c_id = $c_id_ary['id'];
			$query1 = "INSERT INTO tithes (amount, paid_on, c_id) VALUES ('$amount', '$paid_on', '$c_id')";
			mysql_query($query1) or die('could not execute insert query');
			return 0;
		}
		else{
	...
	function cash_in_project($christian_id, $amount){
		...
		if ($rows > 0) {
			...
			$query1 = "INSERT INTO project_offerings (amount, collected_on, c_id, u_id) VALUES ('$amount', '$collected_on', '$c_id', '$u_id')";
			mysql_query($query1) or die('could not execute insert query no login_id');
			return 0;
		}
	}
	function cash_in_htg($amount){
		...
		$collected_on = date('d/m/Y');
		$u_id = $_SESSION['login_id'];
		$query = "INSERT INTO htg_offerings (amount, collected_on, u_id) VALUES ('$amount', '$collected_on', '$u_id')";
	...
	function cash_in_offerings($amount){
		...
		$collected_on = date('d/m/Y');
		$u_id = $_SESSION['login_id'];
		$query = "INSERT INTO regular_offerings (amount, collected_on, u_id) VALUES ('$amount', '$collected_on', '$u_id')";
	...
	function cash_in_sp_offerings($amount, $comment){
		...
		$collected_on = date('d/m/Y');
		$u_id = $_SESSION['login_id'];
		$query = "INSERT INTO other_offerings (amount, collected_on, purpose, u_id) VALUES ('$amount', '$collected_on', '$comment','$u_id')";
		mysql_query($query) or die('could not insert into the regular_offerings table');
	}
	//cash out operations
	function cash_out_salary($amount){
		...
		$withdrawn_on = date('d/m/Y');
		$u_id = $_SESSION['login_id'];
		$query = "INSERT INTO expenses (amount, purpose, withdrawn_on, u_id) VALUES ('$amount', '$purpose', '$withdrawn_on', '$u_id')";
	...
	function recent_transactions(){
		...
		$query = "SELECT * FROM daystatistics";
		$result = mysql_query($query);
		$nbr_rows = mysql_num_rows($result);
		if ($nbr_rows == 1)
		 {
	...
	function statistics_for_period($start_date, $end_date)
	{
		...
		if ($exp_rows > 0) {
			while ($exp_row_data = mysql_fetch_array($exp_result)) {
				...
				$statistics_ass_array['day_of_operation'] = $exp_row_data['withdrawn_on'];
		...
		if ($tithes_rows > 0) {
			while ($tithes_row_data = mysql_fetch_array($tithes_result)) {
				...
				$statistics_ass_array['description'] = 'Total tithes for Sunday '.$tithes_row_data['paid_on'];
				$statistics_ass_array['day_of_operation'] = $tithes_row_data['paid_on'];
		...
		if ($htg_rows > 0) {
			while ($htg_row_data = mysql_fetch_array($htg_result)) {
				...
				$statistics_ass_array['description'] = 'Total HTG for Sunday '.$htg_row_data['collected_on'];
				$statistics_ass_array['day_of_operation'] = $htg_row_data['collected_on'];
				$statistics_ass_array['type_of_operation'] = 'CASH IN';
				array_push($statistics_mult_array, $statistics_ass_array);
			}
		}
		$other_off_query = "SELECT amount, purpose, collected_on FROM other_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date'";
		...
		if ($other_off_rows > 0) {
			while ($other_off_row_data = mysql_fetch_array($other_off_result)) {
				...
				$statistics_ass_array['description'] = $other_off_row_data['purpose'];
				$statistics_ass_array['day_of_operation'] = $other_off_row_data['collected_on'];
				$statistics_ass_array['type_of_operation'] = 'CASH IN';
				array_push($statistics_mult_array, $statistics_ass_array);
			}
		}
		$project_query = "SELECT sum(amount) AS amount, collected_on FROM project_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date' GROUP BY collected_on";
		...
		if ($project_rows > 0) {
			while ($project_row_data = mysql_fetch_array($project_result)) {
				...
				$statistics_ass_array['description'] = 'Total Project Offerings for Sunday '.$project_row_data['collected_on'];
				$statistics_ass_array['day_of_operation'] = $project_row_data['collected_on'];
				$statistics_ass_array['type_of_operation'] = 'CASH IN';
				array_push($statistics_mult_array, $statistics_ass_array);
			}
		}
		$reg_off_query = "SELECT sum(amount) AS amount, collected_on FROM regular_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date' GROUP BY collected_on";
		...
		if ($reg_off_rows > 0) {
			while ($reg_off_row_data = mysql_fetch_array($reg_off_result)) {
				...
				$statistics_ass_array['description'] = 'Total Worship Service Offerings for Sunday '.$reg_off_row_data['collected_on'];
				$statistics_ass_array['day_of_operation'] = $reg_off_row_data['collected_on'];
				$statistics_ass_array['type_of_operation'] = 'CASH IN';
				array_push($statistics_mult_array, $statistics_ass_array);
			}
		}
		// var_dump($statistics_mult_array);
	...
	function quater_income_per_rubric_totals($start_date, $end_date){
		...
		$tithe_income_qry = "SELECT sum(amount) AS amount FROM tithes WHERE paid_on BETWEEN '$start_date' AND '$end_date'";
		...
		$regOff_income_qry = "SELECT sum(amount) AS amount FROM regular_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date'";
		...
		$prjkOff_income_qry = "SELECT sum(amount) AS amount FROM project_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date'";
		...
		$htgOff_income_qry = "SELECT sum(amount) AS amount FROM htg_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date'";
		...
		$otherOff_income_qry = "SELECT sum(amount) AS amount FROM other_offerings WHERE collected_on BETWEEN '$start_date' AND '$end_date'";
	...
	function quater_expenditure_per_rubric_totals($start_date, $end_date){
		...
		$pastorSalary_exp_qry = "SELECT sum(amount) AS amount FROM pastor_salary WHERE withdrawn_on BETWEEN '$start_date' AND '$end_date'";
		...
		$project_exp_qry = "SELECT sum(amount) AS amount FROM project_expenditure WHERE withdrawn_on BETWEEN '$start_date' AND '$end_date'";
		...
		$other_exp_qry = "SELECT sum(amount) AS amount FROM other_expenses WHERE withdrawn_on BETWEEN '$start_date' AND '$end_date'";
